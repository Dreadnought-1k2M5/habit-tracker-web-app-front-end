...
function reducer(previousState, payload){
    let filteredList = payload.filter(elem => elem.habit_data != null); 
...
export default function HabitList(){
    ...
    return (
        <div className="habits-listbox">
            ...
            <div className='habits-listbox-box2'>
                <ul className='habits-listbox-list-habits'>
                {
                        habitListState.map((elemObject, key) => 
                            <li key={`${elemObject.id_habit_table}-${key}`}>
                                <div className='habit-item'>
                                    <div className='habit-title-box'>
                                        <p>
                                            <b>
                                                {elemObject.habit_data}
                                            </b>
                                        </p>
                                    </div>
            ...
            {(isModalClicked.isClicked && isModalClicked.value == "Add") && <ModalComponent isClicked={isModalClicked} clickHandler={setIsModalClicked} /> }
            {(isModalClicked.isClicked && isModalClicked.value == "Edit") && <ModalComponent isClicked={isModalClicked} clickHandler={setIsModalClicked} /> }  
        </div>
    )
}